...
class LogAnalyzer:
    ...
    def get_summary(self) -> str:
        ...
        for d in domains:
            ...
            line = f"| {d_display:<{domain_width}} | {visitors:<10} | {requests:<10} | {downloads:<10} | {err_color}{errors:<10}{Fore.CYAN if errors > 0 else ''} |"
            output.append(Fore.CYAN + line + Fore.RESET)
        output.append(Fore.CYAN + border + Fore.RESET)
        
        if self.bad_actors:
            output.append(f"\n{Fore.YELLOW}| {'Top Bad Actors (IP)':<30} | {'Threat Score':<12} | {'UFW Permanent Ban Command':<48} |{Fore.RESET}")
            bad_border = "-" * 100
    ...
    def get_full_report(self) -> str:
        ...
        for d in domains:
            ...
            report.append(f"{Fore.CYAN} DOMAIN: {Fore.YELLOW}{Style.BRIGHT}{d}{Style.RESET_ALL}")
        ...
        if self.bad_actors:
            ...
            report.append(f"{Fore.RED} IDENTIFIED BAD ACTORS (Threat Detection){Fore.RESET}")
